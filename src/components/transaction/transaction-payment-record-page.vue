  ...
  <div>
    ...
    <el-row>
      <el-table :data="tableData" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中">
        <el-table-column prop="id" label="支付号"  sortable></el-table-column>
        <el-table-column prop="transactionOrderId" label="订单号" sortable></el-table-column>
...
<script>
    export default {
        name: "transaction-payment-record-page",
      data() {
        return {
          tableData: [],
          currentRowDate: {},
      ...
      methods: {
        loadTableData() {
          this.$http.get("/api/transaction-module/get_all_payment_record", {
            params: {
            ...
            this.tableData = data.body.paymentRecordList.list;
            this.total = data.body.paymentRecordList.total;
            this.loading = false;
          })
        },
        ...
        deleteData(rowData) {
          this.$confirm('此操作将永久该记录, 是否继续?', '提示', {
            ...
            this.$http.get("/api/transaction-module/delete_payment_record_by_id", {params: {id: rowData.id}}).then((data) => {
              this.showMessage("success", "操作成功！")
              console.log("successfully! URI : /transaction-module/delete_payment_record_by_id");
              console.log(data.body);
              this.loadTableData();
            }, (response) => {
              this.showMessage("warning", response);
        ...
        showTableDetail(rowData) {
          this.$http.get("/api/transaction-module/get_transaction_order_detail_by_toid", {params: {toid: rowData.transactionOrderId}}).then((data) => {
